...
void BotMgr::LoadConfig(bool reload)
{
    ...
    std::vector<std::string_view> toks5 = Trinity::Tokenize(mult_healing_by_levels, ',', false);
    ...
    {
        Optional<float> val = Trinity::StringTo<float>(toks5[i]);
        if (val == std::nullopt)
            TC_LOG_ERROR("server.loading", "NpcBot.Mult.Healing.Levels contains invalid float value '{}', set to default", toks5[i]);
        float fval = val.value_or(1.0f);
        RoundToInterval(fval, 0.1f, 10.f);
    ...
    std::vector<std::string_view> toks6 = Trinity::Tokenize(mult_hp_by_levels, ',', false);
    ...
    {
        Optional<float> val = Trinity::StringTo<float>(toks6[i]);
        if (val == std::nullopt)
            TC_LOG_ERROR("server.loading", "NpcBot.Mult.HP.Levels contains invalid float value '{}', set to default", toks6[i]);
        float fval = val.value_or(1.0f);
        RoundToInterval(fval, 0.1f, 10.f);
    ...
    std::vector<std::string_view> toks7 = Trinity::Tokenize(mult_mp_by_levels, ',', false);
    ...
    {
        Optional<float> val = Trinity::StringTo<float>(toks7[i]);
        if (val == std::nullopt)
            TC_LOG_ERROR("server.loading", "NpcBot.Mult.MP.Levels contains invalid float value '{}', set to default", toks7[i]);
        float fval = val.value_or(1.0f);
        RoundToInterval(fval, 0.1f, 10.f);
    ...
    for (decltype(toks2)::size_type i = 0; i != toks2.size(); ++i)
    {
        ...
        if (val == std::nullopt)
            TC_LOG_ERROR("server.loading", "NpcBot.WanderingBots.Continents.Levels contains invalid uint32 value '{}', set to default", toks2[i]);
